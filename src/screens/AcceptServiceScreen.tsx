...
import { Table } from '../components/AcceptService/Table';
...
export const AcceptServiceScreen = ({route}: Props) => {
  const {theme: {palette}} = useContext(ThemeContext);
  const {user} = useContext(AuthContext);
  const {status, onGetCollection, datos, total, onRefresh, onShowMore} = useContext(AcceptServiceContext);
  // const [totalRows, setTotalRows] = useState(10);
  const [totalRows, setTotalRows] = useState(dataPerPage);
  ...
  return (
    ...
    >
      <View style={{...optionStyles.mainContainer, backgroundColor: palette.background.default}}>
        <TitleOption title='Aceptacion Orden Servicio'/>
        <Formik
